...
import { evaluateCircadianDecision } from "../../agents/circadian-awareness.js";
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context }) => {
    ...
    const circadian = evaluateCircadianDecision(parsedMessage);
    if (!circadian.allowed) {
      respond(
        false,
        {
          status: "blocked",
          gate: "circadian-awareness",
          kind: circadian.kind,
          phase: circadian.phase,
          localHour: circadian.localHour,
          reason: circadian.reason,
        },
        errorShape(ErrorCodes.INVALID_REQUEST, circadian.reason),
      );
      return;
    }
    const now = Date.now();
